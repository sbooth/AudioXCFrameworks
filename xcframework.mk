...
$(XCFRAMEWORK): $(ARCHIVES)
	rm -Rf "$@"
	xcodebuild -create-xcframework $(foreach xcarchive,$^,-framework "$(xcarchive)/Products/Library/Frameworks/$(FRAMEWORK_NAME).framework" -debug-symbols `pwd`/$(xcarchive)/dSYMs/$(FRAMEWORK_NAME).framework.dSYM ) -output "$@"
